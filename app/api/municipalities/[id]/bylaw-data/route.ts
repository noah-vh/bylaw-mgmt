...
const bylawDataSchema = z.object({
  ...
  max_primary_dwellings: z.number().min(1).default(1),  // Keep default - most lots have 1 primary
  max_adus: z.number().min(0).default(1),  // Keep default - typical ADU ordinance allows 1
  max_total_units: z.number().min(1).default(2),  // Keep default - 1 primary + 1 ADU
  ...
  // Parking
  adu_parking_spaces_required: z.union([z.number().min(0), z.null()]).default(1),  // Keep default - common requirement
