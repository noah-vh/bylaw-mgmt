...
class OshawaScraper(BaseSupabaseScraper):
    """Oshawa municipality scraper"""
    
    def __init__(self, municipality_id: int):
        base_url = "https://www.oshawa.ca"
        search_url = "https://www.oshawa.ca/en/city-hall/by-law-library.aspx"
    ...
    def find_pdf_links(self, html_content: str) -> List[Dict]:
        ...
        # Look for bylaw document links
        for link in soup.find_all('a', href=True):
            href = link['href']
            link_text = link.get_text(strip=True)
            
            # Look for bylaw patterns in href or text
            if (re.search(r'bylaw|by-law|policy|regulation|library', href, re.I) or 
                re.search(r'bylaw|by-law|policy|regulation|library', link_text, re.I)):
    ...
    def handle_pagination(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        next_link = soup.find('a', text=re.compile(r'next|more|view all', re.I))
